...
async function save() {
  ...
  isSaving.value = true
  try {
    await $fetch(`/api/commissions/${props.planId}/assignments`, {
      method: 'POST',
...
<template>
  <UModal
    ...
    description="A configuração passa a valer para este funcionário a partir de agora, se não houver conflito de categoria com outra configuração já atribuída a ele."
