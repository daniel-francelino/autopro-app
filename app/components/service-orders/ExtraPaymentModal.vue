...
async function save() {
  ...
  if (!form.amount || form.amount <= 0) {
    toast.add({ title: 'Informe um valor maior que zero', color: 'warning' })
    return
  }

  isSaving.value = true
  try {
    await $fetch(`/api/service-orders/${props.order.id}/receive-extra-payment`, {
      method: 'POST',
      body: {
        amount: form.amount,
    ...
    toast.add({ title: 'Recebimento registrado com sucesso', color: 'success' })
    ...
    <template #body>
      <div class="space-y-4">
        <p class="text-sm text-muted">
          Registra um recebimento que não precisa bater com nenhuma parcela prevista — vira automaticamente uma linha "Avulso" no plano de pagamento desta OS.
        </p>
        ...
        <UFormField label="Valor recebido" required>
